let watchId = null;
 ...
 async function geocode(strasse, hausnr, ort) {
   try {
     ...
     // 3. Spezielle Behandlung für Hausnummern mit Buchstaben
     // Wenn eine Hausnummer Buchstaben enthält und keine Ergebnisse gefunden wurden,
     // versuche es mit dem numerischen Teil der Hausnummer
     const hasLetters = /^(\d+)([a-zA-Z].*)$/i.test(hausnr);
     
     if (hasLetters) {
       const numericPart = hausnr.match(/^(\d+)/)[1];
       console.log('Hausnummer mit Buchstaben nicht gefunden. Versuche mit Basis-Hausnummer:', numericPart);
       
       // Neuen Cache-Key für die reduzierte Hausnummer erstellen
       const reducedKey = `${normalizedStrasse.toLowerCase()}|${numericPart.toLowerCase()}|${ort.toLowerCase()}`;
       
       // Prüfen, ob die reduzierte Adresse im Cache ist
       if (cache[reducedKey]) {
         console.log('Reduzierte Adresse aus Cache geladen:', reducedKey);
         const resultObj = cache[reducedKey];
         // Speichere auch unter dem Original-Key für zukünftige Anfragen
         cache[key] = resultObj;
         return resultObj;
       }
       
       // Versuche Geocoding mit reduzierter Hausnummer
       const reducedResult = await geocodeNominatim(normalizedStrasse, numericPart, ort);
       
       if (reducedResult) {
         console.log('Gefunden mit reduzierter Hausnummer:', `${strasse} ${numericPart}, ${ort}`);
         const resultObj = {
         ...
         // Speichern im Cache unter beiden Schlüsseln
         cache[key] = resultObj;
       ...
       // Falls auch das nicht funktioniert, versuche es mit dem Original-Straßennamen und reduzierter Hausnummer
       if (normalizedStrasse !== strasse) {
         const originalWithReducedResult = await geocodeNominatim(strasse, numericPart, ort);
         
         if (originalWithReducedResult) {
           console.log('Gefunden mit Original-Straßennamen und reduzierter Hausnummer:', `${strasse} ${numericPart}, ${ort}`);
           const resultObj = {
             coordinates: originalWithReducedResult,
             accuracy: 'approximate',
